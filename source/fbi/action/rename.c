...
static void action_rename_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    ...
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        if(R_SUCCEEDED(res)) {
            if(strncmp(selected->name, "<現在のディレクトリ>", LIST_ITEM_NAME_MAX) != 0 && strncmp(selected->name, "<current file>", LIST_ITEM_NAME_MAX) != 0) {
            ...
            prompt_display_notify("成功", "名前が変更されました。", COLOR_TEXT, NULL, NULL, NULL);
        } else {
            error_display_res(NULL, NULL, res, "名前の変更を実行できませんでした。");
        }
    }
...
void action_rename(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "名前の変更に失敗しました。");
    ...
    kbd_display("新しい名前を入力", ((file_info*) selected->data)->name, SWKBD_TYPE_NORMAL, 0, SWKBD_NOTEMPTY_NOTBLANK, FILE_NAME_MAX, data, action_rename_onresponse);
}