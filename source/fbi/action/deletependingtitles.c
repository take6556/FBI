...
static bool action_delete_pending_titles_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_pending_titles_draw_top, res, "保留中のタイトルを削除できませんでした。");
    return true;
}
...
static void action_delete_pending_titles_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("成功", "保留中のタイトルが削除されました。", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_pending_titles_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("保留中のタイトルの削除", "Bを押してキャンセル", true, data, action_delete_pending_titles_update, action_delete_pending_titles_draw_top);
        } else {
...
static void action_delete_pending_titles_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            error_display_res(NULL, NULL, loadingData->popData.result, "保留中のタイトルのリストの作成に失敗しました。");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "保留中のタイトルのリストの取得中...");
}
...
void action_delete_pending_titles(linked_list* items, list_item* selected, const char* message, bool all) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "削除した保留中のタイトルのデータの割り当てに失敗しました。");
    ...
    if(all) {
        ...
        if(R_FAILED(listRes)) {
            error_display_res(NULL, NULL, listRes, "保留中のタイトルのリストの作成に失敗しました。");
        ...
        info_display("読み込み中", "Bを押してキャンセル.", false, loadingData, action_delete_pending_titles_loading_update, action_delete_pending_titles_loading_draw_top);
    } else {
        ...
        prompt_display_yes_no("確認", message, COLOR_TEXT, data, action_delete_pending_titles_draw_top, action_delete_pending_titles_onresponse);
    }
}
