...
static list_item delete_pending_title = {"保留中のタイトルを消去", COLOR_TEXT, action_delete_pending_title};
static list_item delete_all_pending_titles = {"全ての保留中のタイトルを消去", COLOR_TEXT, action_delete_all_pending_titles};
...
static void pendingtitles_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "保留中のタイトルのアクションデータの割り当てに失敗しました。");
...
static void pendingtitles_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "保留中のタイトルのリストの作成の開始に失敗しました。");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "保留中のタイトルのリストの作成に失敗しました。");
...
void pendingtitles_open() {
    ...
    list_display("保留中のタイトル", "A: 選択, B: 戻る, X: 一覧をリロード", data, pendingtitles_update, pendingtitles_draw_top);
}