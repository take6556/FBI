...
static list_item delete_title_ticket = {"タイトルとチケットを削除", COLOR_TEXT, action_delete_title_ticket};
static list_item extract_smdh = {"SMDHを抽出", COLOR_TEXT, action_extract_smdh};
static list_item import_seed = {"シードをインポ－ト", COLOR_TEXT, action_import_seed};
static list_item browse_save_data = {"セーブデータの閲覧", COLOR_TEXT, action_browse_title_save_data};
static list_item import_save_data = {"セーブデータのインポ－ト", COLOR_TEXT, action_import_twl_save};
static list_item export_save_data = {"セーブデータのエクスポート", COLOR_TEXT, action_export_twl_save};
static list_item erase_save_data = {"データを削除", COLOR_TEXT, action_erase_twl_save};
static list_item import_secure_value = {"セキュアバリューのインポ－ト", COLOR_TEXT, action_import_secure_value};
static list_item export_secure_value = {"セキュアバリューのエクスポート", COLOR_TEXT, action_export_secure_value};
static list_item delete_secure_value = {"セキュアバリューを削除", COLOR_TEXT, action_delete_secure_value};
...
static void titles_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "タイトルのアクションデータの割り当てに失敗しました。");
...
static void titles_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        titles_options_add_entry(items, "ゲームカードを表示", &listData->showGameCard);
        titles_options_add_entry(items, "SDカードを表示", &listData->showSD);
        titles_options_add_entry(items, "NANDを表示", &listData->showNAND);
        titles_options_add_entry(items, "ID順で並び替え", &listData->sortById);
        titles_options_add_entry(items, "名前順で並び替え", &listData->sortByName);
        titles_options_add_entry(items, "大きさ順で並び替え", &listData->sortBySize);
    }
}

static void titles_options_open(titles_data* data) {
    list_display("オプション", "A: 変更, B: 戻る", data, titles_options_update, NULL);
}
...
static void titles_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "タイトルのリストの登録の開始に失敗しました。");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "タイトルのリストの作成に失敗しました。");
...
void titles_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "タイトルのデータの割り当てに失敗しました。");
    ...
    list_display("タイトル", "A: 選択, B: 戻る, X: 一覧をリロード, Select: オプション", data, titles_update, titles_draw_top);
}