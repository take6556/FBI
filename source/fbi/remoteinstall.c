...
static void remoteinstall_network_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(sock >= 0) {
        ...
        if(remoteinstall_network_recvwait(networkData->clientSocket, &size, sizeof(size), 0) != sizeof(size)) {
            error_display_errno(NULL, NULL, errno, "ペイロードの長さを読み取ることができませんでした。");
        ...
        if(remoteinstall_network_recvwait(networkData->clientSocket, urls, size, 0) != size) {
            error_display_errno(NULL, NULL, errno, "URLの読み込みに失敗しました。");
        ...
        error_display_errno(NULL, NULL, errno, "ソケットを開くことに失敗しました。");
...
static void remoteinstall_receive_urls_network() {
    ...
    if(sock < 0) {
        error_display_errno(NULL, NULL, errno, "サーバーソケットを開くことに失敗しました。");
    ...
    if(listen(data->serverSocket, 5) < 0) {
        error_display_errno(NULL, NULL, errno, "サーバーソケットでのリッスンに失敗しました。");

        remoteinstall_network_free_data(data);
        return;
    }

    info_display("URLの受信", "B: 戻る", false, data, remoteinstall_network_update, NULL);
}
...
static void remoteinstall_qr_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(!installData->capturing) {
        ...
        if(R_FAILED(capRes)) {
            ...
            error_display_res(NULL, NULL, capRes, "カメラのキャプチャの開始に失敗しました。");
    ...
    for(int i = 0; i < qrCount; i++) {
        ...
        if(err == 0) {
            ...
            action_install_url("スキャンしたQRコードからインストールしますか？", (const char*) qrData.payload, NULL, NULL, NULL, NULL, NULL);
            return;
        }
    }

    snprintf(text, PROGRESS_TEXT_MAX, "QRコードの読み取りを待っています...");
}
...
static void remoteinstall_scan_qr_code() {
    ...
    if(data->captureInfo.buffer == NULL) {
        error_display(NULL, NULL, "イメージバッファの作成に失敗しました。");
    ...
    info_display("QRコードの読み取り", "B: 戻る, X: カメラ切り換え", false, data, remoteinstall_qr_update, remoteinstall_qr_draw_top);
}
...
static void remoteinstall_manually_enter_urls() {
    kbd_display("URLを入力してください(複数可)", "", SWKBD_TYPE_NORMAL, SWKBD_MULTILINE, SWKBD_NOTEMPTY_NOTBLANK, DOWNLOAD_URL_MAX * INSTALL_URLS_MAX, NULL, remoteinstall_manually_enter_urls_onresponse);
}
...
static void remoteinstall_repeat_last_request() {
    ...
    if(textBuf != NULL) {
        if(remoteinstall_get_last_urls(textBuf, DOWNLOAD_URL_MAX * INSTALL_URLS_MAX)) {
            action_install_url("最後に要求したURLからインストールしますか？", textBuf, NULL, NULL, NULL, NULL, NULL);
        } else {
            prompt_display_notify("失敗", "以前に要求されたURLは見つかりませんでした。", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void remoteinstall_forget_last_request() {
    ...
    if(R_SUCCEEDED(forgetRes)) {
        prompt_display_notify("成功", "最後に要求されたURLの記録を削除しました。", COLOR_TEXT, NULL, NULL, NULL);
    } else {
        error_display_res(NULL, NULL, forgetRes, "最後に要求されたURLの記録の削除に失敗しました。");
    }
}

static list_item receive_urls_network = {"ネットワーク経由でURLを受信する", COLOR_TEXT, remoteinstall_receive_urls_network};
static list_item scan_qr_code = {"QRコードを読み取る", COLOR_TEXT, remoteinstall_scan_qr_code};
static list_item manually_enter_urls = {"手動でURLを入力する", COLOR_TEXT, remoteinstall_manually_enter_urls};
static list_item repeat_last_request = {"最後に要求したURLを繰り返す", COLOR_TEXT, remoteinstall_repeat_last_request};
static list_item forget_last_request = {"最後に要求されたURLの記録を削除する", COLOR_TEXT, remoteinstall_forget_last_request};
...
void remoteinstall_open() {
    list_display("遠隔インストール", "A: 選択, B: 戻る", NULL, remoteinstall_update, NULL);
}
